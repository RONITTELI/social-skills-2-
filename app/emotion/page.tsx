  ...
  const startAnalysis = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        // Play is handled inside analyzeFacialCues or explicitly here, 
        // but analyzeFacialCues waits for play event or playing state.
      ...
      // Start analysis - this promise resolves when video is paused/stopped
      if (videoRef.current) {
        const result = await analyzeFacialCues(videoRef.current);
        
        // Generate feedback once analysis is done
        const feedbackData = {
          ...result,
        };
        ...
        if (aiResult) {
          ...
          // Fallback to local engine if AI fails or no key
